  ...
  return (
    <Container id="download" sx={{pt: { xs: 4, sm: 12 }, pb: { xs: 8, sm: 16 }, position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: { xs: 3, sm: 6 }}}>
      ...
      <Grid container spacing={3} sx={{ alignItems: 'center', justifyContent: 'center', width: '100%' }}>
        {tiers.map((tier) => (
          <Grid size={{ xs: 12, sm: tier.title === 'TouchSlider' ? 12 : 6, md: 4 }} key={tier.title}>
            <Card
            ...
            >
              <CardContent>
                <Box sx={[{mb: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2}, tier.title === 'TouchSlider' ? { color: 'grey.100' } : { color: '' }]}>
                ...
                <Box sx={[ { display: 'flex', alignItems: 'baseline' }, tier.title === 'TouchSlider' ? { color: 'grey.50' }  : { color: null }]}>
                ...
                {tier.description.map((line) => (
                  <Box key={line} sx={{ py: 1, display: 'flex', gap: 1.5, alignItems: 'center' }}>
                    <CheckCircleRoundedIcon sx={[ { width: 20 }, tier.title === 'TouchSlider' ? { color: 'primary.light' } : { color: 'primary.main' }]}/>
                    <Typography variant="subtitle2" component={'span'} sx={[ tier.title === 'TouchSlider' ? { color: 'grey.50' } : { color: null }]}>
                      {line}
                    </Typography>
                  </Box>
                ))}
